...
const DISCOVER_CANDIDATE_GROUPS = process.env.DISCOVER_CANDIDATE_GROUPS === 'true';
...
const CANDIDATE_GROUP_PHRASES = (process.env.CANDIDATE_GROUP_PHRASES ||
    'sexualize minors,sexualizing minors,underage sexual,child sexual abuse,grooming minors,minor exploitation,ageplay')
    .split(',')
...
function upsertVrchatSafetyCandidateGroup(group, matchedTerms) {
...
function getVrchatSafetyCandidateTerms(group) {
    ...
    if (PROTECTIVE_CONTEXT_PHRASES.some(phrase => text.includes(phrase))) {
        return [];
    }

    return CANDIDATE_GROUP_PHRASES.filter(phrase => text.includes(phrase));
...
async function sendVrchatSafetyCandidateAlert(channel, group, matchedTerms) {
    ...
    const embed = new EmbedBuilder()
        .setColor(0xffcc00)
        .setTitle('Unverified VRChat group candidate')
        .setURL(groupUrl)
        .setDescription(
            'A keyword scan found an **unverified** group that may require moderator review.\n\n' +
            `**Group:** [${truncateSafetyText(group.name || groupId, 150)}](${groupUrl})\n` +
            `**ID:** \`${groupId}\`\n` +
            `**Matched terms:** ${matchedTerms.map(term => `\`${term}\``).join(', ')}\n\n` +
            ...
            try {
                ...
                if (DISCOVER_CANDIDATE_GROUPS && candidateChannel) {
                    ...
                    for (const group of publicGroups) {
                        ...
                        const matchedTerms = getVrchatSafetyCandidateTerms(group);

                        if (matchedTerms.length === 0) continue;

                        upsertVrchatSafetyCandidateGroup(group, matchedTerms);

                        if (shouldNotifyVrchatSafetyCandidate(groupId)) {
                            await sendVrchatSafetyCandidateAlert(candidateChannel, group, matchedTerms);
