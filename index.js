...
const PROTECTIVE_CONTEXT_PHRASES = (process.env.PROTECTIVE_CONTEXT_PHRASES ||
    'prevention,awareness,reporting,survivor support,child safety,protect minors,anti-grooming,education,resources')
    .split(',')
    .map(phrase => phrase.trim().toLowerCase())
    .filter(Boolean);
...
const VRCHAT_CANDIDATE_GROUP_DETAIL_DELAY_MS = Math.max(
    0,
    Number.parseInt(process.env.VRCHAT_CANDIDATE_GROUP_DETAIL_DELAY_MS || '250', 10) || 250
);
...
async function requestVrchatApiJson(method, endpointPath, {
    ...
    if (response.status === 429) {
        throw new Error('VRChat rate limited this request. Try again later.');
    }
...
function getVrcLoggerCommandList() {
    ...
    return [
        ...
        '`!safetyscan`, `!scanmembergroups`, `!blacklistgroup`',
...
function getVrchatBlacklistGroupUsage() {
    ...
    return [
        ...
        'Example: `!blacklistgroup add grp_00000000-0000-0000-0000-000000000000 grooming ban Reviewed evidence summary`',
...
async function handleVrchatBlacklistGroupCommand(message, args) {
    ...
    const category = String(workingArgs.shift() || '').trim().toLowerCase();
    const severity = String(workingArgs.shift() || '').trim().toLowerCase();
    const evidence = workingArgs.join(' ').trim();
    const groupId = extractVrchatGroupIdFromInput(groupInput);

    if (!groupId || !category || !severity || !evidence) {
        return message.reply(getVrchatBlacklistGroupUsage());
    }
    ...
    try {
        const group = await fetchVrchatGroup(groupId);
        groupName = group?.name || groupName;
        groupUrl = group?.url || groupUrl;
    } catch (error) {
...
function addVrchatCandidateMatch(matches, term, field, type) {

    const key = `${type}:${term}:${field}`;

    if (matches.seen.has(key)) return;

    matches.seen.add(key);
    matches.items.push({
        term,
        field,
        type
...
function getVrchatSafetyCandidateMatch(group) {
    ...
    for (const part of parts) {
        ...
        const text = normalizeVrchatSafetyScanText(part.text);

        if (!text) continue;

        for (const term of PROTECTIVE_CONTEXT_PHRASES) {
            if (textContainsSafetyTerm(text, term)) {
                protectiveTerms.add(term);
            }
        }

        for (const phrase of CANDIDATE_GROUP_PHRASES) {
            if (textContainsSafetyTerm(text, phrase)) {
                addVrchatCandidateMatch(matches, phrase, part.field, 'phrase');
            }
        }

        for (const term of HIGH_RISK_MINOR_SAFETY_TERMS) {
            if (textContainsSafetyTerm(text, term)) {
                addVrchatCandidateMatch(matches, term, part.field, 'high-risk');
            }
        }

        const minorTerms = MINOR_REFERENCE_TERMS.filter(term => textContainsSafetyTerm(text, term));
        const harmTerms = MINOR_HARM_CONTEXT_TERMS.filter(term => textContainsSafetyTerm(text, term));

        if (minorTerms.length > 0 && harmTerms.length > 0) {

            for (const minorTerm of minorTerms.slice(0, 5)) {
                addVrchatCandidateMatch(matches, minorTerm, part.field, 'minor-reference');
            }

            for (const harmTerm of harmTerms.slice(0, 5)) {
                addVrchatCandidateMatch(matches, harmTerm, part.field, 'harm-context');
            }
...
async function getVrchatSafetyCandidateScanGroup(group, groupDetailCache) {

    const groupId = group?.groupId || group?.id;

    if (!groupId || !VRCHAT_CANDIDATE_SCAN_DETAIL_LOOKUP) return group;

    if (!groupDetailCache.has(groupId)) {

        const detail = await fetchVrchatGroup(groupId).catch(error => {
        ...
        if (VRCHAT_CANDIDATE_GROUP_DETAIL_DELAY_MS > 0) {
...
async function sendVrchatSafetyCandidateAlert(channel, group, candidateMatch, member = null) {
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(
            ...
            `**ID:** \`${groupId}\`\n` +
            `**Matched terms:** ${matchedTerms.slice(0, 25).map(term => `\`${term}\``).join(', ')}\n\n` +
...
function formatVrchatSafetyScanResult(result) {
    ...
    return `Scan complete: checked ${result.checked}/${result.totalMembers} members; ` +
        `${result.flaggedMembers} member alert(s), ${result.candidateGroups} candidate group alert(s), ` +
...
async function runVrchatSafetyScan({
    ...
    try {
        ...
        const candidateChannel = DISCOVER_CANDIDATE_GROUPS
        ...
        for (const member of members) {
            ...
            try {
                ...
                if (DISCOVER_CANDIDATE_GROUPS && candidateChannel) {
                    ...
                    for (const group of publicGroups) {
                        ...
                        const scanGroup = await getVrchatSafetyCandidateScanGroup(group, candidateGroupDetailCache);
                        const candidateMatch = getVrchatSafetyCandidateMatch(scanGroup);
...
async function handleSafetyScanMessageCommand(message, args) {
    ...
    if (subcommand !== 'run') {
        return message.reply('Usage: `!safetyscan status` or `!safetyscan run`');
    }

    const statusMessage = await message.reply('Starting VRChat safety scan...');
    ...
    try {
        ...
        await statusMessage.edit(`Safety scan failed: ${truncateText(error.message, 300)}`);
...
\`!blacklistgroup add group category severity evidence\` - Staff: adds a reviewed VRChat group.
